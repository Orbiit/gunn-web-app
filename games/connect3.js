(function (root, factory) {
  ...
  function connect3() {
    ...
    class Tile {
      constructor(x, y, colour, entering = false) {
        ...
        this.element.addEventListener("click", e => {
          ...
          if (!selectedTile) {
            this.element.classList.add("connect3-active");
            selectedTile = {
              x: this.x,
              y: this.y,
              moveTileTo: (x, y) => {
                this.moveTo(x, y);
                this.element.classList.remove("connect3-active");
              },
              cancel: () => {
                this.element.classList.remove("connect3-active");
                selectedTile = null;
        ...
        switch (colour) {
          case "bomb":
            this.element.classList.add("connect3-bomb");
          default:
            this.element.classList.add("connect3-" + shapes[colour]);
            this.element.style.backgroundColor = colours[colour];
            this.colour = colour;
        }

        if (entering) this.element.classList.add("connect3-entering");
      ...
      remove(animate = true, addNewItem = true) {
        if (animate) {
          this.element.classList.add("connect3-exiting");
    ...
    tiles = [],
    tileElems = [];
    squareElem.classList.add("connect3-grid");
    for (let y = 8; y--;) {
      let row = [];
