...
async function main () {
  ...
  const inputPath = fileURLToPath(new URL('../json/clubs-links.json', import.meta.url))
  const links = await fs.readFile(inputPath, 'utf-8')
    .then(JSON.parse)
    .catch(() => ({}))
  ...
  $('table tbody tr:not(:first-child)').each(function () {
    ...
    const { video, zoom, time: time2, president: president2, signup } = links[name] || {}
    const actualTime2 = time2 && time2.replace(/(MON|TUES|WED|THURS|FRI)(\/(MON|TUES|WED|THURS|FRI))?/, '')
    if (links[name]) {
  ...
  const outputPath = fileURLToPath(new URL('../json/clubs.json', import.meta.url))
