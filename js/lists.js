window.addEventListener(
  ...
  e => {
    ...
    ajax(
      ...
      e => {
        ...
        for (
          ...
          innerHTML += `<li tabindex="0" data-person="${person}" data-search="${person} ${
            staff[person].jobTitle
          } ${staff[person].department ||
            ''}"><span class="primary">${person}</span><span class="secondary">${
            staff[person].jobTitle
          }</span><span class="secondary">${(staff[person].email || '').replace(
            'pausd.org',
            '...'
          )}</span></li>`
        }
      ...
      e => {
        stafflist.innerHTML = `<li class="error">${e}${localize(
          'staff-error'
        )}</li>`
      }
    )
    ...
    stafflist.addEventListener(
      ...
      e => {
        ...
        `
            ...
            let innerHTML = ''
            if (person.jobTitle)
              innerHTML += `<p><strong>${localize('title')}</strong> ${
                person.jobTitle
              }</p>`
            if (person.department)
              innerHTML += `<p><strong>${localize('department')}</strong> ${
                person.department
              }</p>`
            if (person.email)
            ...
            if (person.phone)
              innerHTML += `<p><strong>${localize('phone')}</strong> ${
                person.phone
              }</p>`
            if (person.webpage)
              innerHTML += `<p><strong>${localize(
            ...
            if (person.oc)
              innerHTML += `<p><strong>${localize(
                'basement'
              )}</strong> <a href="https://sheeptester.github.io/hello-world/elements.html" target="_blank" rel="noopener noreferrer">${localize(
                'oc-basement'
              )}</a></p>`
    ...
    const staffSearchValue = /(?:\?|&)staff-search=([^&]+)/.exec(
    ...
    ajax(
      ...
      e => {
        clublist.innerHTML = `<li class="error">${e}${localize(
          'club-error'
        )}</li>`
      }
    )
    ...
    window.showClub = clubName => {
      ...
      innerHTML += `<p><strong>${localize('advisors')}</strong> ${
        club.teacher
      }</p>`
      innerHTML += `<p><strong>${localize(
        'teacher-email'
        ...
        club.email
      }" target="_blank" rel="noopener noreferrer">${club.email}</a></p>`
      if (club.donation)
        innerHTML += `<p><strong>${localize('donation')}</strong> ${
          club.donation
        }</p>`
    ...
    const clubSearchValue = /(?:\?|&)club-search=([^&]+)/.exec(
      window.location.search
    )
    if (clubSearchValue) {
